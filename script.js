...
function toggleHeader() {
  ...
  if (headerContent.style.display === 'flex' || headerContent.style.display === '') {
...
document.querySelectorAll('.read-more').forEach(button => {
  button.addEventListener('click', (event) => {
    ...
    Swal.fire({
      ...
      title: `<div style="font-size: 1.5rem; text-align: center;">${title}</div>`, // Center title
      html: `
        <div style="text-align: center;"> <!-- Center the form content -->
          <img src="${image}" alt="${title}" style="width: 100%; max-width: 300px; height: auto; object-fit: contain; border-radius: 8px; margin-bottom: 1rem;">
          <p style="margin-top: 1rem; font-size: 0.9rem; color: #ccc;">
            <strong>${author}</strong> <span style="color: #888;">| ${date}</span>
          </p>
          <p style="font-size: 1rem; color: #ccc;">${content}</p>
          ...
          <div style="display: flex; flex-direction: column; align-items: center;">
            <input id="swal-input-name" class="swal2-input" placeholder="Your Name" style="width: 80%; margin-bottom: 10px;">
            <input id="swal-input-email" class="swal2-input" placeholder="Your Email" style="width: 80%; margin-bottom: 10px;">
            <textarea id="swal-input-message" class="swal2-textarea" placeholder="Your Message" style="width: 80%;"></textarea>
          </div>
        </div>
      `,
      showCancelButton: true,
      confirmButtonText: 'Submit',
      confirmButtonColor: '#28a745', // Customize confirm button color
      cancelButtonText: 'Close',
      cancelButtonColor: '#dc3545', // Customize cancel button color
      preConfirm: () => {
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire({
          ...
          confirmButtonText: 'OK',
          background: '#333', // Dark background for success alert
